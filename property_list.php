...
<?	
	...
	setcookie("error_msg",'1',time () - 30, "/");
	}
	if (isset ($_COOKIE['success']))
	{
	echo "<p>Спасибо за покупку =)</p>";
	setcookie("success",'1',time () - 30, "/");
	}
	...
	$min='0';
	$max='0';
	$query = "SELECT * FROM property WHERE deleted='0' and user_id_prop>?  and address LIKE ? limit 25";
	...
	$stmt->execute();
			...
			foreach($rows as $value)
			{
				...
				printf("<tr><td><form action='php/buy.php' method='POST'>
						...
						$value['user_id_prop'],$value['cost'],$value['address']
				);
				if ($min=='0') $min=$value['user_id_prop'];
			}
			printf("</table>");
			setcookie('prev',$min);
			setcookie('next',$value['user_id_prop']);
